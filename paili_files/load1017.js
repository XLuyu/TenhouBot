...
function img1(n,style){
	return "<img src=\"t/"+n+".gif\" border=0 "+(style?style:"")+" />";
}
function img2(n){
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
function printTehai(qt,q){//qt="q"/"p" q=string
	var s="";
	s+="<hr size=1 color=#CCCCCC >";
	switch(qt.substr(0,1)){
	case "q":s+="標準形(七対国士を含む)の計算結果"    +" / <a href=\"?p"+qt.substr(1)+"="+q+"\">一般形</a><br>";break;
	case "p":s+="一般形(七対国士を含まない)の計算結果"+" / <a href=\"?q"+qt.substr(1)+"="+q+"\">標準形</a><br>";break;
	}
	var no14=(qt.substr(1,1)=="d");
	qt=qt.substr(0,1);

	q=MPSZ.expand(q);//展开成标准描述2246m -> 2m2m4m6m
	q=q.substr(0,14*2);
	var c=MPSZ.exextract136(q);
	var tsumo136=-1;
	while(tsumo136=Math.floor(Math.random()*136), c[tsumo136]);//随机生成一张不属于手牌的牌
...
//		s+="<br>time="+tt+" eval="+SYANTEN.n_eval;
	...
	if (v[34] && v[34].n){
		...
		trtd.push(v[34]);
		tehai+="<br><br><a href=\"?"+qt+"="+v[34].q+"\">次のツモをランダムに追加</a>";
	}
...
function butOnclick(){//按钮点击事件
	printTehai("q",document.f.q.value);
	return false;
}
document.write(
	"<form id=fs name=f style=\"margin:0px;\" >"+
		"<a href=\"?\" >新規</a> | "+
		"手牌 <input id=tiles type=text name=q size=25 >"+
		"<input id=submit type=submit value=\" OK \"  onclick=\"javascript:butOnclick();\">"+
	...
	"<hr size=1 color=#CCCCCC >"+
	"<table align=center cellpadding=0 cellspacing=0><tr><td>この牌姿のURL : <input type=text size=45 value=\""+location.href+"\"></td></tr></table>"+
	"<hr size=1 color=#CCCCCC >"+
	...
	"- 有効牌をクリックすると打牌後にその牌をツモ牌として表示します<br>"+
